...
export class OrdersService {
  ...
  async create(userId: string, createOrderDto: CreateOrderDto): Promise<Order> {
    ...
    // Получаем ID всех продуктов из заказа
    ...
    // Создаём Map для быстрого доступа к продукту по ID
    const productMap = new Map(products.map((p) => [p.id, p]));

    // Проверяем наличие и рассчитываем сумму
    let total = 0;
    ...
    for (const item of items) {
      ...
      if (!product) {
        // на всякий случай, хотя уже проверили выше
      ...
      // Сохраняем данные для OrderItem
      orderItemsData.push({
    ...
    try {
      ...
      // Уменьшаем остатки продуктов
      for (const item of items) {
        const product = productMap.get(item.productId);
        if (!product) {
          // Этого не должно случиться, но TypeScript требует проверки
          throw new BadRequestException(
  ...
  async findOne(id: string, userId?: string, role?: UserRole): Promise<Order> {
    ...
    // Проверка прав: админ может смотреть любой заказ, пользователь только свои
